...
if [ "$EUID" -ne 0 ]; then
    echo "注意: 此脚本需要 root 权限来安装 udev 规则"
    echo "请输入密码或使用 sudo 运行"
    echo
fi
...
mkdir -p "$CONFIG_DIR"
if [ -d "configs" ]; then
    cp configs/*.cfg.example "$CONFIG_DIR/" 2>/dev/null || true
...
echo "使用方式:"
echo "  ./build.sh           # 编译程序"
echo "  ./spacefn \$HOME/.config/spacefn/default.cfg  # 运行"
echo
echo "首次使用请重新登录以使组权限生效"
